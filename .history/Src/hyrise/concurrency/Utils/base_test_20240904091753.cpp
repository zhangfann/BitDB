...
// }
void EXPECT_TABLE_EQ(const Table &tleft, const Table &tright, bool order_sensitive) {
  EXPECT_TRUE(_table_equal(tleft, tright, order_sensitive));
}
...
// }
void ASSERT_TABLE_EQ(const Table &tleft, const Table &tright, bool order_sensitive) {
  ASSERT_TRUE(_table_equal(tleft, tright, order_sensitive));
}
...
::testing::AssertionResult _table_equal(const Table &tleft, const Table &tright, bool order_sensitive) {
  Matrix left = _table_to_matrix(tleft);
  Matrix right = _table_to_matrix(tright);
  // compare schema of tables
  //  - column count
