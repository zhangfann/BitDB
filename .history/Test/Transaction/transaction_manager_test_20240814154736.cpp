...
TEST(NonActiveTransactionCannotBeAborted) {
  ...
  ASSERT_THROW(manager().abort(*context), std::logic_error);
...
TEST( CommitShouldCommitAllFollowingPendingTransactions) {
  ...
  EXPECT_TRUE(prev_last_commit_id == manager().last_commit_id());

  manager().commit(*context_1);

  EXPECT_TRUE(context_2->commit_id() == manager().last_commit_id());
