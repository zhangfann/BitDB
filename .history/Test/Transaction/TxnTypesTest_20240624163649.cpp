...
TEST(PutTest) {
  ...
  delete t;
    ...
    std::map<Key, Value> m2 ={{1,1}};
  p.NewTxnRequest(new Expect(m2));  // Should abort (wrong value for key)
  t = p.GetTxnResult();
  EXPECT_EQ(ABORTED, t->Status());
  delete t;

    std::map<Key, Value> m3 ={{1,2}};
  p.NewTxnRequest(new Expect(m3));  // Should commit
  t = p.GetTxnResult();
  EXPECT_EQ(COMMITTED, t->Status());
  delete t;
