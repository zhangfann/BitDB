...
class OperatorsTableScanTest : public BaseTest, public ::testing::WithParamInterface<EncodingType> {
 protected:
  void SetUp() override {
    ...
    auto int_int_7 = load_table("../Src/hyrise/resources/test_data/tbl/int_int_shuffled.tbl", ChunkOffset{7});
    auto int_int_5 = load_table("../Src/hyrise/resources/test_data/tbl/int_int_shuffled_2.tbl", ChunkOffset{5});
  ...
  std::shared_ptr<TableWrapper> get_int_float_op() {
    return load_and_encode_table("../Src/hyrise/resources/test_data/tbl/int_float.tbl");
  }

  std::shared_ptr<TableWrapper> get_int_sorted_op() {
    return load_and_encode_table(
        "../Src/hyrise/resources/test_data/tbl/int_sorted.tbl", ChunkOffset{4},
  ...
  std::shared_ptr<TableWrapper> get_int_only_null_op() {
    return load_and_encode_table(
        "../Src/hyrise/resources/test_data/tbl/int_only_null.tbl", ChunkOffset{4},
        std::make_optional(std::vector<SortColumnDefinition>{SortColumnDefinition(ColumnID(0), SortMode::Ascending)}));
  }

  std::shared_ptr<TableWrapper> get_int_string_op() {
    return load_and_encode_table("../Src/hyrise/resources/test_data/tbl/int_string.tbl");
  }

  std::shared_ptr<TableWrapper> get_int_float_with_null_op(const ChunkOffset chunk_size = ChunkOffset{2}) {
    return load_and_encode_table("../Src/hyrise/resources/test_data/tbl/int_float_with_null.tbl", chunk_size);
  }
  ...
  std::shared_ptr<const Table> create_referencing_table_w_null_row_id(const bool references_dict_segment) {
    const auto table = load_table("../Src/hyrise/resources/test_data/tbl/int_int_w_null_8_rows.tbl", ChunkOffset{4});
...
TEST_P(OperatorsTableScanTest, DoubleScan) {
  const auto expected_result = load_table("../Src/hyrise/resources/test_data/tbl/int_float_filtered.tbl", ChunkOffset{2});
