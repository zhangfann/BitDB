...
void leaf_node_insert(Cursor *cursor, uint32_t key, Row *value) {
	...
	if (num_cells >= LEAF_NODE_MAX_CELLS) {
		// 如果leaf中已经满了 则需要split
		// Node full
		leaf_node_split_and_insert(cursor, key, value);
		return;
	}

	// 从末尾至cursor指向的位置 依次向后移动一位
	if (cursor->cell_num < num_cells) {
...
//根据每个node容纳的数量 将老node中cell重新分布入新node和老node
//并将新cell插入对应位置
void leaf_node_split_and_insert(Cursor *cursor, uint32_t key, Row *value) {
	...
	/*
	 *获取老的页面 \old_node, Page.get_page
	 *获取一个新的页面 \new_node, Page.get_page
	 * */
	...
	for (int32_t i = LEAF_NODE_MAX_CELLS; i >= 0; i--) {
		// destination_node, 当前的page
//		_分割右侧 destination_node指向新page
//		_分割左侧 destination_node指向老page
